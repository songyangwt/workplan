...
public String execute() throws Exception {
		...
		/*if(yearmonth.compareTo("201501")<0||yearmonth.compareTo("209912")>0)
		{
	    	...
	    	JieDiaoDAO wddao = new JieDiaoDAO();
	    	...
	    	try {
				...
				for (int i = 1; i < nn; i++) {
						 ...
						 wdtemp.setZu(Integer.valueOf(sheet.getCell(1, i).getContents().trim()));
						 wdtemp.setBegindate(sheet.getCell(2, i).getContents().trim());
						 wdtemp.setEnddate(sheet.getCell(3, i).getContents().trim());
						 wdtemp.setLocation(sheet.getCell(4, i).getContents().trim());
